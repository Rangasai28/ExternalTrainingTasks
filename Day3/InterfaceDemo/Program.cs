namespace InterfaceDemo
{
    ...
    public class FlightInquiryPlatform
    {
        public  List<Flight> flights;
        public void flightDetails(Flight flight)
        {
            Console.WriteLine(flight.printDetails());
        }

        public void delayStatus(Flight flight)
        {
            Console.WriteLine(flight.FlightDelayStatus(2));
            Console.WriteLine();
        }

        public void baggageALlowance(Flight flight)
        {
        ...
        public void specificFlight(Flight flight)
        {
            
                if(flight is DomesticFlight dom)
                {
                    Console.WriteLine(dom.UpdateBaggageAllowance());
                   
                }
                if(flight is InternationalFlight Inf)
                {
                    Console.WriteLine(Inf.visaStatus());
                    
                }
               
                if(flight is BusinessClassFlight Busi)
                {
                    Console.WriteLine(Busi.UpgradeClass());
    ...
    public  class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            FlightInquiryPlatform Inquiry = new();
                
            Inquiry.flights =  new List<Flight>() { Domflight, InterFlight, Businessflight };

            Inquiry.specificFlight(Domflight);
            Inquiry.specificFlight(InterFlight);
            Inquiry.specificFlight(Businessflight);
